...
/*   Updated: 2024/09/28 21:48:41 by opanikov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3D.h"

void render_wall_slice(t_data *data, int x, int drawStart, int drawEnd, int texX, double texPos, double step, int *texture_data)
{
	int	y;
	int	texY;
	unsigned int color;
...
double calculate_initial_texture_position(int drawStart, int lineHeight, int screenHeight)
{
	return (drawStart - screenHeight / 2 + lineHeight / 2) * calculate_texture_step(lineHeight);
}
int *select_texture(t_data *data, double wallX, int *texX)
{
	int *texture_data;
